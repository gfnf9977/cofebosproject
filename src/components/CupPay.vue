  ...
  <div class="modal-overlay" v-if="isModalOpen" @click.self="closeModal">
    <div class="modal-content">
      ...
      <form @submit.prevent="submitOrder">
        ...
        <button type="submit" class="submit-btn" @click="setPaymentType('points')" :disabled="!hasEnoughPoints">Сплатити 70 балів</button>
        <button type="submit" class="submit-btn" @click="setPaymentType('cash')">Сплатити 450 грн при отриманні</button>
      </form>
...
export default {
  ...
  data() {
    return {
      ...
      postomatNumber: '',
      address: '',
      orderDetails: 'чашка', // Pre-filled with "чашка"
      ...
      stores: [],
      paymentType: 'points', // Default payment type
  ...
  methods: {
    ...
    async submitOrder() {
      const orderData = {
        ...
        paymentType: this.paymentType, // Include payment type in order data
      };
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.isModalOpen = false; // Directly set isModalOpen to false
          this.$emit('close'); // Optional: emit close event if needed

          // Update user points if payment type is 'points'
    ...
    async updateUserPoints() {
      const pointsToDeduct = 70; // Number of points to deduct
      try {
