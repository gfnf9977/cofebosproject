<template>
  <div class="change-password-content">
    ...
    <div class="password-container">
      <input :type="currentPasswordType" v-model="currentPassword" placeholder="Введіть поточний пароль" />
      <button @click="toggleCurrentPasswordVisibility" class="toggle-password">
        <span v-if="currentPasswordVisible" class="icon">👁️</span>
...
export default {
  ...
  methods: {
    ...
    async verifyCredentials() {
      try {
        const response = await axios.post('http://localhost:5000/api/login-check', {
        ...
        console.error('Помилка при перевірці логіну та пароля:', error);
        this.message = 'Сталася помилка при перевірці логіну та пароля.';
      }
    },
